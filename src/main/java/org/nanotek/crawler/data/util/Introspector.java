...
import org.nanotek.beans.EventSetDescriptor;
import org.nanotek.beans.FeatureDescriptor;
import org.nanotek.beans.IndexedPropertyDescriptor;
...
import org.nanotek.beans.SimpleBeanInfo;
import org.nanotek.beans.ThreadGroupContext;
            ...
            for (Map.Entry<String,PropertyInfo> entry : ClassInfo.get(this.beanClass).getProperties().entrySet()) {
                try {
					addPropertyDescriptor(null != entry.getValue().getIndexed()
					        ? new IndexedPropertyDescriptor(entry.getKey(), entry.getValue().getPropertyType())
					        : new PropertyDescriptor(entry.getKey(), entry.getValue().getPropertyType()));
